...
| Classification               | **Core — REINSTATED 2026-07-17.** Was incorrectly downgraded to Peripheral on 2026-07-16 on the reasoning that D-004's cross-cycle (2015-2016-to-2017+) bridging resolves this concern. That reasoning was flawed: D-004 only harmonizes modern NHANES cycles against each other: it does nothing to address the separate, more fundamental question of whether D-001's NHANES-III-trained coefficients require adjustment when applied to *any* modern, non-Jaffe-biased creatinine data, regardless of which modern cycle. Reclassification corrected following user's detailed technical review and independent confirming simulation. |
...
| Planned Resolution           | This requires a substantive methodological decision before implementation, not a documentation fix: either (a) apply a compensating adjustment to modern creatinine values to approximate the NHANES-III (Jaffe-biased) scale D-001's coefficients were fit on, or (b) accept the ~1-2 year systematic bias as a known, documented limitation of the replication and report it prominently in the thesis's Limitations section, or (c) empirically test both approaches against BioAge package output (which faces the identical NHANES-III-training-bias question) to see which produces better-calibrated results. Cross-check against Paper_002_BioAge.md's Section 16 "corrected according to the analytical notes posted by NHANES" statement, which suggests BioAge's own team may have already made a decision on this exact question worth investigating directly. |
...
| Classification               | Core - blocks final confidence in D-001, though a provisional default (0.09165, the erratum value) is documented and in use |
...
| Planned Resolution           | Provisional default: retain 0.09165 (the erratum's value) in D-001, on the basis that a formally published journal correction notice is the more authoritative source for the specific equation it was issued to correct. Resolve empirically during Validation_Protocol.md Check 2: compute Phenotypic Age using both constants on a subsample and compare each against BioAge package output - whichever constant produces closer agreement is very likely the one BioAge itself uses, providing indirect but strong resolution. |
...
| Scientific Impact          | **Critical - highest magnitude of any open gap.** Independent simulation (2026-07-19): a participant genuinely aged 87 but coded as 80 shows a Phenotypic Age approximately 6.1 years lower than their true-age computation; a genuinely-90-year-old shows an ~8.8 year understatement. This substantially exceeds the magnitude of EG-004 (~1-2 years) and EG-010 (~1.8 years). NHANES's own documentation notes the weighted mean true age of the 80+ top-coded group is approximately 85 in recent cycles - meaning this is not a rare edge case affecting only extreme outliers, but a systematic, sizable understatement affecting the entire 80+ subgroup, which is disproportionately represented in aging research given PhenoAge's target population. |
| Classification               | Core - blocks unqualified use of Phenotypic Age scores for the 80+ subgroup; also distorts Validation Protocol Check 1's correlation-based face-validity check (a "vertical wall" of top-coded participants at age=80 with varying true PhenoAge would flatten/distort the expected linear relationship, understating the correlation coefficient) |
...
| Planned Resolution           | Per the user's proposed options: (a) exclude RIDAGEYR == 80 participants from the primary analytic sample entirely, reporting this exclusion and its rationale prominently in the thesis Limitations section; or (b) retain them but flag with a dedicated indicator variable, report Phenotypic Age results for this subgroup separately/with an explicit caveat, and exclude them specifically from Validation Protocol Check 1's correlation calculation (to avoid the vertical-wall distortion) while still including them in Check 2 (BioAge comparison, which faces an identical top-coding constraint and is therefore a fair comparison). Option (b) is recommended over (a) since it preserves sample size and directly documents the limitation rather than silently shrinking the cohort. |
