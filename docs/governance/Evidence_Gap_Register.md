...
| 0.16    | 2026-07-19 | EG-013 closed (converted to D-008) - the exact source URLs were recorded and all coefficients were directly verified word-for-word against official CDC documentation. |
...
| 0.22 | 2026-07-23 | Replaced tool-specific verification attribution with neutral evidence-provenance wording; Evidence Gap status and scientific interpretation are unchanged. |
...
| Review Status                 | **Closed — Converted to Decision (D-008)**, 2026-07-19. User supplied the exact official CDC/NCHS URLs (BIOPRO_J.htm, HSCRP_J.htm). Both sources were retrieved directly, and every previously provisional coefficient was independently verified word-for-word against the "Analytic Notes" sections — all were confirmed correct. The equations are now recorded in Replication_Protocol.md Section 8 as confirmed, cited primary-source values, not provisional ones. |
